...
let noWebGL=false;
export type SpinMotion={rotation:T.Quaternion;axis:T.Vector3;trail:T.Vector3[];revision:number};
export type ViewerProps={motion?:SpinMotion;settings:Simulation;diameters:[number,number];bridge:number;left:boolean;model:Simulation['modelVersion'];valid:boolean;drill:boolean;view:'grip'|'core';showRemoved:boolean;shellOpacity:number};
export default function CoreViewer(p:ViewerProps){const host=useRef<HTMLDivElement>(null),[error,setError]=useState('');
useEffect(()=>{const el=host.current;if(!el)return;let renderer:T.WebGLRenderer|SoftwareRenderer|undefined,controls:OrbitControls|undefined,resize:ResizeObserver|undefined;const scene=new T.Scene();const labels:{element:HTMLSpanElement;position:T.Vector3}[]=[];let disposed=false,animation=0;setError('');
...
const spinAxis=p.motion?new T.Mesh(new T.CylinderGeometry(.75,.75,260,8),new T.MeshBasicMaterial({color:0xffffff,depthTest:false})):null;const trailDots=p.motion?Array.from({length:61},()=>{const m=new T.Mesh(new T.SphereGeometry(1.8,8,6),new T.MeshBasicMaterial({color:0xffb24f,depthTest:false}));m.renderOrder=5;group.add(m);return m;}):[];if(spinAxis){group.add(spinAxis);spinAxis.renderOrder=5;}
if(p.view==='core')camera.position.set(260,160,330);else camera.position.set(0,0,450);controls.update();const draw=()=>{if(disposed)return;if(p.motion){group.quaternion.copy(p.motion.rotation);spinAxis?.quaternion.setFromUnitVectors(new T.Vector3(0,1,0),p.motion.axis);trailDots.forEach((m,i)=>{m.visible=!!p.motion!.trail[i];if(m.visible)m.position.copy(p.motion!.trail[i]);});}renderer!.render(scene,camera);for(const label of labels){const point=label.position.clone().applyQuaternion(group.quaternion).project(camera);label.element.style.left=`${(point.x+1)*el.clientWidth/2}px`;label.element.style.top=`${(1-point.y)*el.clientHeight/2}px`;label.element.hidden=point.z>1||point.z< -1;}};controls.addEventListener('change',draw);resize=new ResizeObserver(()=>{camera.aspect=el.clientWidth/el.clientHeight;camera.updateProjectionMatrix();renderer!.setSize(el.clientWidth,el.clientHeight);draw();});resize.observe(el);draw();if(p.motion){let revision=-1;const animate=()=>{if(p.motion!.revision!==revision){revision=p.motion!.revision;draw();}animation=requestAnimationFrame(animate);};animation=requestAnimationFrame(animate);}
}catch{setError('3D 계산에 실패했습니다. 수치를 바꾸거나 WebGL 지원 브라우저에서 다시 열어주세요.');}
return()=>{disposed=true;cancelAnimationFrame(animation);resize?.disconnect();controls?.dispose();labels.forEach(l=>l.element.remove());const geometries=new Set<T.BufferGeometry>(),materials=new Set<T.Material>();scene.traverse(o=>{if(o instanceof T.Mesh){geometries.add(o.geometry);(Array.isArray(o.material)?o.material:[o.material]).forEach(m=>materials.add(m));}});geometries.forEach(g=>g.dispose());materials.forEach(m=>m.dispose());renderer?.dispose();renderer?.domElement.remove();};
},[p.settings,p.diameters,p.bridge,p.left,p.model,p.valid,p.drill,p.view,p.showRemoved,p.shellOpacity,p.motion]);
