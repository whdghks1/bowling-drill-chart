import {z} from 'zod';
export const simulationSchema=z.object({version:z.literal(1),productId:z.string().max(160),weight:z.number().int().min(6).max(20),coreTilt:z.number().min(-180).max(180),coreTurn:z.number().min(-180).max(180),middleDepth:z.number().min(10).max(90),ringDepth:z.number().min(10).max(90),middleForward:z.number().min(-20).max(20),ringForward:z.number().min(-20).max(20),middleSide:z.number().min(-20).max(20),ringSide:z.number().min(-20).max(20),pitchMode:z.enum(['neutral','degrees']),modelVersion:z.literal('c3-v1').nullable()});
