...
import {simulationSchema} from './simulation.ts';
export type Measure = number | string | null;
export const gripItems=[['finger','원핸드 · 엄지 사용'],['thumbless','덤리스'],['twohand','투핸드'],['conventional','원핸드 · 기존 컨벤셔널'],['two','엄지 없음 · 기존 차트']];
export const fingerGripItems=[['unknown','미지정'],['fingertip','핑거팁'],['semi','세미 핑거팁'],['conventional','컨벤셔널']];
export const fingerGripLabel=(value:string)=>fingerGripItems.find(([key])=>key===value)?.[1]||'미지정';
export const insertItems=[['GNS','GNS · 기존 선택'],['G&S Ice','G&S Ice'],['G&S X-900','G&S X-900'],['G&S Black','G&S Black'],['VISE P/O','VISE P/O'],['VISE P/S','VISE P/S'],['VISE O/PO','VISE O/PO'],['Turbo Quad','Turbo Quad'],['Turbo MS Quad','Turbo MS Quad'],['Turbo Quad Classic','Turbo Quad Classic'],['Turbo Classic-Pro','Turbo Classic-Pro'],['Turbo Quad2','Turbo Quad2'],['Turbo Power-SB','Turbo Power-SB'],['없음','없음']];
...
export const newSpec=():Chart['spec']=>({version:2,fingerGrip:'unknown',date:new Date().toLocaleDateString('sv-SE'),driller:'',weight:null,serial:'',spanBasis:'cut',pitchReference:'',papHorizontal:null,papVertical:null,layout:'',ovalShape:'round',ovalLong:null,ovalShort:null,ovalAngle:null,ovalReference:'그립 중심선 0° · 볼 표면을 바라보고 시계방향',depthReference:'볼 표면에서 홀 바닥까지, 드릴 축 방향 실측',finish:''});
...
export function normalizeChart(raw:Partial<Chart>):Chart{const base=emptyChart();return {...base,...raw,middle:{...newHole(),...raw.middle},ring:{...newHole(),...raw.ring},thumb:{...newHole(),...raw.thumb},spec:{...newSpec(),...(!raw.spec?{spanBasis:'unknown' as const,ovalShape:'unknown' as const,depthReference:''}:{}),...raw.spec,fingerGrip:raw.spec?.fingerGrip??(raw.grip==='conventional'?'conventional':'unknown')}};}
