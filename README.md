...
공개 차트는 누구나 로그인 없이 조회할 수 있습니다. 로그인 화면에서 이름·비밀번호로 가입하면 일반 회원이 됩니다. 이름은 2~40자이며 공백과 대소문자, 유니코드 호환 문자를 정규화해서 중복을 검사합니다. 비밀번호는 8~128자이고 scrypt(개별 무작위 salt) 해시로 저장합니다.
