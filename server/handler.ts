 ...
 return async function handle(req:Request,context:{ip?:string}={}){
  ...
  try{
   if(path==='/login'&&req.method==='POST'){
    try{assertConfigured(env);}catch{return response({error:'관리자 로그인 설정이 필요합니다.',code:'AUTH_CONFIG'},503);}
    const raw=await req.text();if(raw.length>4096)return response({error:'요청이 너무 큽니다.'},413);
    const parsed=z.object({role:z.literal('admin'),password:z.string().min(1).max(512)}).safeParse(JSON.parse(raw));if(!parsed.success)return response({error:'로그인 정보를 확인해주세요.'},400);
