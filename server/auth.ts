...
export type AuthEnv={ADMIN_PASSWORD?:string;CLUB_PASSWORD?:string;SESSION_SECRET?:string};
export const COOKIE='bowling_fit_session';
export function assertConfigured(env:AuthEnv){if((env.ADMIN_PASSWORD?.length??0)<12||(env.CLUB_PASSWORD?.length??0)<8||(env.SESSION_SECRET?.length??0)<32||env.ADMIN_PASSWORD===env.CLUB_PASSWORD)throw Error('AUTH_CONFIG');}
...
export const passwordFor=(role:Role,env:AuthEnv)=>role==='admin'?env.ADMIN_PASSWORD!:env.CLUB_PASSWORD!;
...
export function readSession(cookie:string|null,env:AuthEnv,now=Date.now()):Role|null{try{assertConfigured(env);const token=cookie?.split(';').map(s=>s.trim()).find(s=>s.startsWith(COOKIE+'='))?.slice(COOKIE.length+1);if(!token||token.length>2048)return null;const [data,sig,...extra]=token.split('.');if(!data||!sig||extra.length||!equals(sign(data,env),sig))return null;const payload=JSON.parse(Buffer.from(data,'base64url').toString('utf8'));if(!['admin','club'].includes(payload.role)||!Number.isFinite(payload.exp)||payload.exp<=now||payload.exp>now+12*60*60*1000||payload.v!==version(payload.role,env))return null;return payload.role;}catch{return null;}}
